        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              type="danger"
            >关闭评论</el-button>
            <el-button @click="toggleStatus(scope.row)" v-else size="small" type="success">打开评论</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    // 获取评论列表
    async getComments() {
      // 调用articles接口，但是response_type=comment,获取评论系那个管字段数据。
    ...
    toggleStatus(row) {
      ...
      // 键值对传参：?article_id=文章ID
      // 请求体传参：allow_comment=想修改的状态
      // 关闭打开提示不一样：
      // 关闭：您是否确定要关闭该文章的评论功能，关闭后用户无法对文章进行评论。
      // 打开：您是否确定要打开该文章的评论功能。
      // row 中有 comment_status 标识当前文章状态
      ...
      this.$confirm(row.comment_status? text1 : text2, '温馨提示', {
          ...
          try {
              // res.data.data.allow_comment 修改后的状态
              // 请求
